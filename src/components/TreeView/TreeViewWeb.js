...
export default class Treeview extends Component {
  ...
  nodeEditForm = (label, paramObj, parent, index, parameterType) => {
    return (
  ...
  makeChildren = (node, label) => {
    ...
    children = node.map((paramObj, index) => {
      ...
      if (paramObj.children.length > 0 && paramObj.showChildren) {
        let babies = this.makeChildren(paramObj.children, label);
        let normalMode = (
          <div className="node">
        ...
        item = (
          <li key={index} onClick={(e) => { e.stopPropagation(); this.toggleView(paramObj) }}>
            {(paramObj.editMode) ? this.nodeEditForm(label, paramObj, node, index) : normalMode}
            {babies}
          </li>
        )
      }
  ...
  getNodes = () => {
    this.state.firstNode = true;
    if (typeof this.state.data.methodName === 'undefined') return null;
    let children = this.makeChildren(this.state.data.params, Constant.LABEL_PARAM);
    return children;
  }
  getStepNodes = () => {
    this.state.firstNode = true;
    if (typeof this.state.data.methodName === 'undefined') return null;
    let children = this.makeChildren(this.state.data.steps, Constant.LABEL_STEP);
    return children;
  }
